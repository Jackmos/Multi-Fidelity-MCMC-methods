...
class Inversion_tinyDA(BayesianLibraryBase):
    ...
    def HPO_tinyDA(self, 
        ...
        def evaluate_model(**model_kwargs: Any) -> float:
            ...
            _, error, _ = self.forward_NN.param_inverse(
                ...
                cov_likelihood=calculate_cov_likelihood(model_kwargs.get('sigma', support["sigma"]), 
    ...
    def run_inverse_tiny(self, 
        ...
        _, observation = Inversion_tinyDA.process_data(inputs_HF, real_parameters, x_obs, output_HF)
...
class BayesianInverseProblem_NN:
    ...
    def set_algorithm(self, algorithm_name: str, forward_NN: 'INetwork' = None) -> None:
        ...
        if forward_NN is None:
            self.strategy = AlgorithmFactory.create_algorithm(algorithm_name, self.forward_NN)
        else:
            self.strategy = AlgorithmFactory.create_algorithm(algorithm_name, forward_NN)
...
class MH_tinyAlgorithm(ProposalAlgorithm):
    ...
    def setup_proposal(self, *args, **kwargs) -> object:
        ...
        # Assuming GaussianRandomWalk is defined elsewhere in your code
...
# class ProposalAlgorithm(ABC):
#     __slots__ = ['levels', 'subsampling_rate', '_parameters_cache']

#     def __init__(self, levels: int, subsampling_rate: Optional[Union[int, Tuple[int, int]]] = None):
#         self.levels = levels
#         self.subsampling_rate = subsampling_rate
...
#             self._parameters_cache = self._compute_parameters(*args, **kwargs)
#         return self._parameters_cache

#     @abstractmethod
#     def _compute_parameters(self, *args, **kwargs) -> Dict[str, Union[float, np.ndarray]]:
#         pass
###############################à
...
class MCMC_cuqi:
    ...
    def __call__(
        ...
        for i, result in enumerate(results):
            estimates[:, i] = result[0].flatten()  
            chains[i, :, :] = result[1].squeeze()  
            post[:, i * (N - burn_in):(i + 1) * (N - burn_in)] = result[2]  
