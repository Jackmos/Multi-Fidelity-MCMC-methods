# Example III: First scenario, one parameter case
import numpy as np
import keras
...
def main_function():
    ...
    # example selection 
    cuqi_example=False          # True, to generate the example related to CUQIpy
    DA_comp_example=True        # True, to show the DA vs AM comparison
    ...
    (t_HF,y_HF,mu_HF)=solver.generate_dataset(T=8.0, params=np.array(params), h=0.02, fidelity='HF')

    solver.plot_dataset()


    (t_LF,y_LF,mu_LF)=solver.generate_dataset(T=8.0, params=np.array(params), h=0.05, fidelity='LF')
    ...
    (t_valhf,y_valhf,mu_valhf)=solver.generate_dataset(T=8.0, params=np.array(params_val), h=0.02, fidelity='HF')

    (t_valLF,y_valLF,mu_valLF)=solver.generate_dataset(T=8.0, params=np.array(params_val), h=0.05, fidelity='LF')




    # manipulations to give proper shape and type
    ...
    # NETWORK DEFINITION
    N=[400,1000]            # epochs
    n=[200,250]             # batch size
    names = ['LF', 'HF']    # network types and order
    ...
    # build NN
    final_model_trained = NetworkFactory.build_network(NetworkConfig(**definition_2steps),solver.getModel)
    ...
    System1.save_plots_per_param(x_HF,y_HF,y_test_pred,final_model_trained)
