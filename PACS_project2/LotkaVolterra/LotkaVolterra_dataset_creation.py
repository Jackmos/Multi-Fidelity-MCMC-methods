...
def generate_dataset(T, mu_values, h):
    num_steps = int(T / h)
    ...
    for mu in mu_values:
        ...
        for i in range(num_steps):
            y_values[i + 1, :] = runge_kutta_2nd_order(y_values[i, :], time_points[i], h, mu)

        y_values_list.append(y_values)

    return time_points, y_values_list, mu_values

def save_dataset(time_points, y_values_list, mu_values, filename='dataset_mu_variationLF.csv'):
    data = []
...
T = 10.0
h = 0.25
mu_values = np.linspace(1, 3, 5)  # Change the number of values as needed

# Generate and save dataset for different values of mu
time_points, y_values_list, mu_values = generate_dataset(T, mu_values, h)
save_dataset(time_points, y_values_list, mu_values)
