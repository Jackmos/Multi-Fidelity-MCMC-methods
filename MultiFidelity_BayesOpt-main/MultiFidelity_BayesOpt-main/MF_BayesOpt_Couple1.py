...
if validation:
    early_stopping = tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=patience, restore_best_weights=True)   # val_loss = loss on validation data
else:
...
if not MATLAB:
    ...
    plt.plot(xhf_test, y_test * scale, label = 'HF', color = 'red')
    plt.plot(xhf_test, mean_sim * scale, label = 'mean', color = 'blue')
    plt.fill_between(xhf_test, ((mean_sim - std_sim) * scale)[:,0], ((mean_sim + std_sim) * scale)[:,0], color='lightblue', alpha=0.5, label='mean $\pm$ std')
    plt.plot(xhf,y * scale, 'r*')
    plt.plot(xhf_val, np.mean(np.array(pred_sim_val), axis = 0).flatten()*scale, 'o')
    plt.xlabel('x')
    plt.legend()
