...
#     # elif algo=="MLDA":
#     #     # puoi metterci MAP usando solo la posterior relativa al caso ottimale 
#     #     # vedi esempio libreria con numero diverso di dati 
...
def MCMC(my_posterior, N, burnin, n=1, diagnostic=True, rwmh_cov=None, rmwh_scaling=0.1, period=100, t0=0, rwmh_adaptive=False, algo="MH", dim=0):
    ...
    if algo == "MH":
        ...
        my_proposal = tda.MLDA(posteriors=my_posterior, subsampling_rates=[5], adaptive_error_model='state-independent', initial_parameters=MAP, store_coarse_chain=True, proposal=tda.AdaptiveMetropolis(C0=rwmh_cov))
        """
        Parameters
        ----------
        posteriors : list
        ...
        proposal : tinyDA.Proposal
            Transition Q                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 for coarsest MCMC proposals.
        ...
        my_chains = tda.sample(my_posterior, my_proposal, iterations=N, n_chains=n, initial_parameters=MAP, subsampling_rate=5)
    else:
        raise ValueError("Unknown algorithm %s" % algo)
