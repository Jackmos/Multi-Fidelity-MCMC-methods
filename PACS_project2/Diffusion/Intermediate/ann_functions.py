...
def getModel(params,name):
    if(name == '2step'):
        ...
        inputs = Input(shape=(2,)) #second NN (NN_Lin) in the 3-steps architecture
        ...
        inputs = Input(shape=(3,)) #third NN (NN_HF) in the 3-steps architecture
        hidden1 = Dense(int(params['nodes']),activation='tanh',kernel_regularizer=l2(params['l2weight']),kernel_initializer=params['kernel_init'])(inputs)
        output = Dense(1,activation='linear',name='HF')(hidden1)
        ...
        hidden1 = Dense(64,activation='tanh',kernel_initializer=params['kernel_init'])(inputs) # The same input layer is used for high and low fidelity data
        hidden2 = Dense(64,activation='tanh',kernel_initializer=params['kernel_init'])(hidden1)
        outputLF = Dense(1,activation='linear',name='LF')(hidden2) # low-fidelity output is situate at the third hidden layer.
