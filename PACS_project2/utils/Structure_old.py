...
class Neural_Network(INetwork):
    ...
    def set_inputs(self, x_data: np.ndarray ) -> None:
        """
        Function created to set the inputs in case of multilevel scenario
        Args:
            x_data (np.ndarray): The input data for the model.
        """
        self.inputs = x_data 

    def _set_level(self,  prev_steps: List, level: int = 1) -> None:  # x_data: np.ndarray, prev_steps: List, level: int = 1) -> None:            
        """
...
class MultiFidelity(INetwork):
    ...
    def multilevel_observations(self, levels:int,
                                y_obs:np.ndarray,
        ...
        for l in range(levels-1):
            self.model_list[l].set_inputs(t_eval)
            y.append(self.model_list[l]._wrapper_prediction(parameters))# np.hstack((t_eval,np.tile(parameters,t_eval.shape)))))                   #################
