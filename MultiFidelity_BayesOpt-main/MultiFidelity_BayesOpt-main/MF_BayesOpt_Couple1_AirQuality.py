...
df = pd.read_csv('dfTrain_AirQuality.csv', index_col=0)[:11995] 
...
original_fidelity_order_train = [lowfid0_train, lowfid1_train, lowfid2_train, lowfid3_train, lowfid4_train]
original_fidelity_order_val = [lowfid0_val, lowfid1_val, lowfid2_val, lowfid3_val, lowfid4_val]
original_fidelity_order_test = [lowfid0_test, lowfid1_test, lowfid2_test, lowfid3_test, lowfid4_test]
original_fidelity_labels = [0, 1, 2, 3, 4]
...
y = highfid_train.reshape(-1,1)
y_val = highfid_val.reshape(-1,1)
y_test = highfid_test.reshape(-1,1)
...
params = {'lr' :5e-3, 
                ...
                'activation' : 'tanh',
                'layers_encoder' : [],
                'layers_decoder' : [16, 16, 16], 
                'model_type_encoder' : 'Dense',
                'model_type_decoder' : 'Dense',
                'l2weight' : 1e-4, 
                'Nepo' : Nepo,
                'concatenate' : False}
if validation:
...
for n_model in range(n_models):
    ...
    #Preprocess data
    x = fidelity_order_train[n_model].reshape(-1,1)
    x_val = fidelity_order_val[n_model].reshape(-1,1)
    x_test = fidelity_order_test[n_model].reshape(-1,1)
