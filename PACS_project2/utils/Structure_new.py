        ...
        def wrapper(self, *args, **kwargs):
            ...
            if key not in cache:
                if len(cache) >= maxsize:
                    cache.pop(next(iter(cache)))  # Remove the oldest cached entry
...
class NetworkFactory:
    ...
    @staticmethod
    def build_network(config: NetworkConfig) -> 'INetwork':
        """
        ...
        if network_type_enum in {
            ...
            return NetworkFactory._build_neural_network(config)
        
        elif network_type_enum == NetworkType.STEP:
            return NetworkFactory._build_multifidelity_network(config)
        
        elif network_type_enum == NetworkType.INTER:
            return NetworkFactory._build_intermediate_network(config)
        
        elif network_type_enum in {
            NetworkType.LSTM, NetworkType.LSTM_SUPPORT, NetworkType.LSTM_SUPPORT2
        }:
            return NetworkFactory._build_lstm_network(config)
    ...
    @staticmethod
    def _build_neural_network(config: NetworkConfig) -> 'Neural_Network':
        """
    ...
    @staticmethod
    def _build_multifidelity_network(config: NetworkConfig) -> 'MultiFidelity':
        """
    ...
    @staticmethod
    def _build_lstm_network(config: NetworkConfig) -> 'LSTM_network':
        """
    ...
    @staticmethod
    def _build_intermediate_network(config: NetworkConfig) -> 'Intermediate':
        """
...
class INetwork(ABC):
    ...
    def param_inverse(self, 
        ...
        if observation is None and x_real is not None:
            observation = simulate_observations(x_real, cov_noise, self._wrapper_prediction)
        else:
            observation = [perturbation(y, cov_noise).flatten() for y in observation] if isinstance(observation, list) else perturbation(observation, cov_noise).flatten()
        ...
        if levels > 1:
            ...
            my_loglike = [setup_likelihood(observation[l], cov_likelihood, cov_noise, dim) for l in range(levels)]
            my_posterior = [tda.Posterior(my_prior, my_loglike[i], self.model_list[i]._wrapper_prediction) for i in range(levels)]
        else:
            my_loglike = setup_likelihood(observation, cov_likelihood, cov_noise, dim)
    ...
    def inverse_cuqi(self, 
        ...
        if observation is not None:
            dim_obs, range_geometry = check_observation_shape(observation)
            if dim_obs is None:
                return
        else:
            warnings.warn("No observation nor data given", UserWarning)
        ...
        m = x_real.shape[0]
        A = initialize_model(self._wrapper_prediction, proposal_algorithm, range_geometry, m)

        x = Gaussian(mean=mean_prior, cov=cov_prior)
        y = Gaussian(A(x), sqrtcov=proposal_sd)

        observation = perturbation(observation, sd_noise)
...
class Neural_Network(INetwork):
    ...
    def training(
        ...
        self.hist = self.model.fit(train_dataset, epochs=epoch, verbose=0, callbacks=[callbacks] if callbacks else None)
    ...
    def HPO(self, data_val: np.ndarray, output_val: np.ndarray, num_trials:int=15) -> Dict[str, Any]:
        ...
        def objective(trial):
            ...
            with tf.device(self.device if self.device else '/GPU:0'):  # Default to GPU if not specified and an appropriate GPU is present                
                loss = kCrossVal_parallel(self._N, data_val, output_val,                        # Perform k-fold cross-validation to evaluate the model
...
class MultiFidelity(INetwork):
 ...
 #       K.clear_session()
    ...
    def training(self, device: str = None, profiler: Optional[tf.keras.callbacks.TensorBoard] = None):
        """
    ...
    def HPO(self, device: str = None, profiler: Optional[tf.keras.callbacks.TensorBoard] = None,
...
class LSTM_network(INetwork):
    ...
    def training(self, seq_length: int, seq_freq: int, epoch: int) -> tf.keras.callbacks.History:
        ...
        input_train_seq, output_train_seq = self._sliding_windows(    # -> self?
