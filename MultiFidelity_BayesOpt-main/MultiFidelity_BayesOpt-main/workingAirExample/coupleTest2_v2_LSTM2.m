...
numLFs = 8-1; %number of low fidelity datasets, LSTM2 takes always time as first!
vars = [optimizableVariable('numUsedInputs', [1, numLFs], 'Type', 'integer')];
evaluationTimes = [];

for i = 1:numLFs
        vars = [vars, ...
            optimizableVariable(['fidelityX', num2str(i)], [0, numLFs], 'Type', 'integer'), ...
        ];
end
...
for i = 1:300
    ...
    save('tempreswtimes.mat', 'contX', 'contY', 'evaluationTimes')
...
sensorOrder = sensorOrder + 1 %add one because we skipped sensor X0
%% functions
...
function [objective,coupledconstraints, userdata] = loss_function_helper(x)
    ...
    sensorOrder = flip(sensorOrder);
    %disp(x)
    [last_goodness, all_goodness] = pyrunfile("MF_BayesOpt_Couple1_AirQuality_LSTM2.py", ["goodness" "all_goodness_val"], fidelity_order_labels=sensorOrder, externalInput=1);
